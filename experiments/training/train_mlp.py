...
def parse_args():
	...
	parser.add_argument('-h_layers', nargs='+', type=int, default=[32, 16])
	return parser.parse_args()

def get_save_path(antipattern, net_number):
	return os.path.join(ROOT_DIR, 'neural_networks', 'mlp', 'trained_models', antipattern, 'network' + str(net_number))

def build_dataset(systems, antipattern):
	X = np.empty(shape=[0, nnUtils.get_nb_metric(antipattern)])
	Y = np.empty(shape=[0, 1])
	for system in systems:
		X = np.concatenate((X, nnUtils.get_instances(system, antipattern)), axis=0)
...
if __name__ == "__main__":
	...
	# Create dataset
	x_train, y_train = build_dataset(training_systems, args.antipattern)
	x_test,  y_test  = build_dataset(test_systems, args.antipattern)

	tf.reset_default_graph()

	# Create model
	model = mlp.MLP(args.h_layers, x_train.shape[-1])
	...
	with tf.Session() as session:
		...
		for i in range(args.n_net):
			...
			saver.restore(sess=session, save_path=get_save_path(args.antipattern, i))
